...
--            self[y][x] = 0
--        end
--        --print( tostring(y).." : ".. table.concat(GCommon.Craft.Grid[y]))
--    end
--end
function Grid:ctor()
    self.w = 6
...
function Grid:locationOfPosition(p,isshow)
    ...
    return cc.p(self:xLocationOfPosition(p,isshow)+self.offset.x,self:yLocationOfPosition(p,isshow)+self.offset.y)-- + block.offset;
end
--得到可用的空间
...
function Grid:finding(tile)
    ...
    --         end
    --         return (a.pos.y<b.pos.y and a.pos.x<b.pos.x  )
    --     end
    -- table.sort(close_list,comps)
